	...
	for (final HashMap<String, Object> map : list) {
		...
		public void call(Row t) throws Exception {
			...
			if (indexMap.containsKey(display[i])) {
			    disValueBuilder.append(t.get(i)).append(",");
			} else {
			    disValueBuilder.append("'").append(t.get(i)).append("'").append(",");
			}
		    }
		    String dispaly = displayBuilder.toString();
		    String disValue = disValueBuilder.toString();
		    insertSql.append("insert into offline_basic_analysis.report_").append(map.get("id")).append("(").append(dispaly.substring(0, dispaly.lastIndexOf(",")))
			    .append(")").append(" values").append("(").append(disValue.substring(0, disValue.lastIndexOf(","))).append(");");
		    System.out.println(insertSql);
		    db.insertSql(insertSql.toString());
		}
	    });
	    String endTime = DateUtil.timeStampToDate(System.currentTimeMillis());
	    String sql = "update offline_basic_analysis.customize_report_info set creat_time='" + startTime + "',update_time='" + endTime + "' , status='Y' where id="
		    + map.get("id");
	    mu.insertSql(sql);
	}
	ctx.stop();
